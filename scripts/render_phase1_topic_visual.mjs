...
const FLAGS = ['--script', '--scene-plan', '--timing-manifest', '--aspect', '--output', '--report', '--stills-dir', '--clips-dir'];
...
export function parseArgs(argv) {
  ...
  if (FLAGS.some((key) => !out[key])) throw Error('arguments_required');
  return out;
}
...
export async function main(argv = process.argv.slice(2)) {
  ...
  const durationInFrames = Math.round(inputProps.duration_seconds * 30);
  const landscape = aspect === '16:9', width = landscape ? 1920 : 1080, height = landscape ? 1080 : 1920;
