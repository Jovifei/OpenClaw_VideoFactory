...
### Topic-only qualification compatibility

For `topic_only_v1`, use only a validated `local_subject` handoff and require
Flash/watchdog, FreeRTOS, I2C, plus one distinct live topic. Each final review
must bind the audible-preview SHA-256 and Jovi's explicit approval. This scope
does not weaken the local-reference route or permit automatic export/publication.
