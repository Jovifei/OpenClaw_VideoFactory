...
# 2026-08-24 - Speech cues need measured audio boundaries

- A visual pulse must be attached to the measured start of the exact phrase it explains, not to a looping frame counter or a guessed scene interval. Split narration at semantic phrases when word timestamps are unavailable, then use the resulting TTS clip starts as the visual cue source of truth.
