...
def test_flash_watchdog_fixture_uses_reviewed_topic_copy() -> None:
    ...
    assert set(script["beats"][2]["fact_refs"]) == {"iwdg_independent_timeout","service_window_is_budget"}
...
def test_freertos_fixture_covers_reviewed_facts_and_uses_only_freertos_cards() -> None:
    brief = load_local_brief(ROOT / "examples/phase1_local_freertos/brief.json")
    plan = build_local_plan(brief, ROOT)
    script = plan["script"]
    expected_fact_ids = {"mutex_task_ownership","isr_nonblocking_boundary","priority_inheritance_context","short_isr_handler"}
    used_fact_ids = {str(fact_id) for beat in script["beats"] for fact_id in beat.get("fact_refs", [])}
    assert used_fact_ids == expected_fact_ids
    assert len(script["beats"]) == 5
    assert all("freertos" in beat["required_tags"] for beat in script["beats"])
    assert all("freertos_mutex_plain" in beat["required_tags"] for beat in script["beats"])
    selections = plan["asset_selection"]["selections"]
    selected_ids = [str(item["asset_id"]) for item in selections]
    selected_paths = [str(item["relative_path"]) for item in selections]
    assert len(selected_ids) == 5
    assert len(set(selected_ids)) == 5
    assert all(asset_id.startswith("pink_pig.freertos_mutex_plain_") for asset_id in selected_ids)
    assert all("freertos_mutex_plain_illustrations/" in path for path in selected_paths)
    assert not any("modbus_rtu_illustrations/" in path for path in selected_paths)
    assert not any("flash_watchdog" in path for path in selected_paths)
    assert all(item["source_type"] == "deterministic_diagram" for item in selections)
    assert plan["mascot_mode"] == "off"
    assert all("layout_mode" not in scene for scene in plan["storyboard"]["scenes"])
...
def test_generic_script_removes_duplicate_claim_punctuation(tmp_path: Path) -> None:
    ...
    brief = {"schema_version":"1.0","input_mode":"topic","topic":topic,"factual_brief":{"schema_version":"1.0","topic_digest":digest,"review_status":"verified","facts":[{"fact_id":"fact_one","claim":"这是一个带句号的事实。","source_ids":["source_one"]}],"sources":[{"source_id":"source_one","title":"测试资料","publisher":"测试机构","url":"https://example.test/source","kind":"official_document"},{"source_id":"source_two","title":"补充资料","publisher":"测试机构","url":"https://example.test/source-2","kind":"standard"}]}}
    plan = build_local_plan(brief, ROOT)
