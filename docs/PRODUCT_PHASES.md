...
4. `local_subject`：仅供受控 SQLite 内部入口；只有 `fixture_id=local_subject`
   且严格 subject review package 有效时，prereview 才会将其规范化为 `topic`。
...
### Topic-only qualification scope

`topic_only_v1` 是当前的附加、fail-closed 验收范围：Flash/watchdog、FreeRTOS、
I2C 与一条不重复的 live topic。它可明确省略 reference job，但任何实际提供的
reference job 仍须按完整 rights、analysis、originality 与 human-review 合同验证。
每个 prereview 必须为 `ready`，且人工 `approved` 审阅必须与 audible-preview
SHA-256 精确绑定。该范围不删除 `legacy_topic_reference_v1` 能力。
